...
def load_model(config, args):
    model_name = config.get("model_mapping", {}).get(args.model)  # Updated to Falcon-7B-Instruct
...
#         chat_history.append(config.get("prompt_module", {}).get('general')+ "\n")
#     chat_history.append("user message : " + user_input + "\n")
#     prompt = " ".join(chat_history)
#     response = process_text(prompt, tokenizer,model)
#     chat_history.append("AI expert :" + response + "\n")
#     return response
